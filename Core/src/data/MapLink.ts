...
export class MapLinkDataController extends DataControllerNumber<MapLink> {
    ...
    public getRandomExcept(mapLinkId: number): MapLink {
        let generatedMapLink = this.getRandomLink();
        if (generatedMapLink.id === mapLinkId) { // If the player is already on the destination, get the inverse link
            generatedMapLink = this.getInverseLinkOf(mapLinkId);
        }
