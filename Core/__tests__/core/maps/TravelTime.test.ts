...
describe('TravelTime', () => {
	...
	beforeEach(() => {
		...
		// Mock logs
		vi.spyOn(draftBotInstance.logsDatabase, 'logTimeWarp').mockResolvedValue(undefined as any);
		vi.spyOn(draftBotInstance.logsDatabase, 'logAlteration').mockResolvedValue(undefined as any);
	...
	it('gets travel data with small events', async () => {
		...
		expect(result.nextSmallEventTime).toBe(Math.max(start, lastEvent.time, start + effectDurationMs) + 200_000);
	});
	...
	it('timeTravel moves dates and logs', async () => {
		...
		expect(player.effectEndDate.valueOf()).toBe(now + 5_000 - 60_000);
		expect(player.startTravelDate.valueOf()).toBe(now + 5_000 - 60_000);
		expect(small.time).toBe(now + 3_000 - 60_000);
