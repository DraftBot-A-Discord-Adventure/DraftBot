...
export async function interactOtherPlayersCollector(packet: ReactionCollectorCreationPacket, context: PacketContext): Promise<void> {
	...
	await DiscordCollectorUtils.createAcceptRefuseCollector(interaction, embed, packet, context, translateEmojiToDiscord(DraftBotIcons.collectors.interactPoorCoin));
}