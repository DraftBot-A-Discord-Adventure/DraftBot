...
import {BaseGuildTextChannel, TextChannel, User} from "discord.js";
...
import {NotificationsTypes, NotificationType} from "./NotificationType";
				...
				if (keycloakUser && keycloakUser.attributes.discordId) {
					...
					draftBotClient.users.fetch(discordId).then(async (discordUser) => {
						await NotificationsHandler.sendNotification(
							...
							notificationType,
							i18n.t(notificationContent, {lng, interpolation: {escapeValue: false}}),
							lng
						);
					});
				}
