                        ...
                        if (defender.id === message.author.id) { // le defenseur et l'attaquant sont la même personne
                            ...
                            if (defenderPlayer.level < DefaultValues.fight.minimalLevel) {
                                ...
                                if (playerManager.checkState(defenderPlayer, message, ":smiley:", language, defender.username)) {  //check if the player is not dead or sick or something else
                                    ...
                                    fight(lastMessageFromBot, message, actualUser, player, actuelPlayer, opponentPlayer, opponentUser, attacker, defender, defenderPlayer, attackerPower, defenderPower, defenderDefenseAdd, attackerDefenseAdd, lastEtatFight, attackerSpeedAdd, defenderSpeedAdd, nbTours, logMessage);
                                }
                            }
                        }
                    }
...
/**
 * Create the message which display the live informations of the user in the fight
 * @param {*} message 
 * @param {*} actualUser 
 * @param {*} actuelPlayer 
 * @param {*} actualUserPoints 
 * @param {*} opponentUser 
 * @param {*} opponentPlayer 
 * @param {*} opponentUserPoints 
 */
const createLastEtatMessage = function (message, actualUser, actuelPlayer, actualUserPoints, opponentUser, opponentPlayer, opponentUserPoints) {
    let embed = new Discord.RichEmbed();
    embed.setDescription(Text.commands.fight.attackerEmoji + actualUser.toString() +
        ...
        Text.commands.fight.endLine + Text.commands.fight.defenderEmoji + opponentUser.toString() +
        Text.commands.fight.statusPoints + opponentUserPoints + Text.commands.profile.statsAttack + opponentPlayer.attack +
        Text.commands.profile.statsDefense + opponentPlayer.defense + Text.commands.profile.statsSpeed + opponentPlayer.speed).setTitle(
            Text.commands.fight.startStart + actualUser.username + Text.commands.fight.startJoin + opponentUser.username + Text.commands.fight.startEnd);
    return embed;
}

/**
 * Edit the message which contains the last 5 logs from the fight
 * @param {*} data - The log to add
 */
function editLogMessage(data, oldEmbed) {
    const embed = new Discord.RichEmbed();
    let oldDesc = oldEmbed.embeds[0].description;
    let nbOfLine = oldDesc.split("\n").length;
    if (nbOfLine >= 5) oldDesc = ""; //if the logs are sup or equal to 5
    embed.setDescription(oldDesc + "\n" + data);
    embed.setTitle(oldEmbed.embeds[0].title)
    oldEmbed.edit(embed);
}

/**
 * Create the message which will contain all the logs from the fight
 * @param {*} data - The first log
 */
function createLogMessage(data) {
    const embed = new Discord.RichEmbed().setTitle(data).setDescription("...");
    return embed;
}
 ...
 * @param {*} nbTours
 */
async function fight(lastMessageFromBot, message, actualUser, player, actuelPlayer, opponentPlayer, opponentUser, attacker, defender, defenderPlayer, attackerPower, defenderPower, defenderDefenseAdd, attackerDefenseAdd, lastEtatFight, attackerSpeedAdd, defenderSpeedAdd, nbTours, logMessage) {
    ...
    if (lastEtatFight === undefined) {
        lastEtatFight = await message.channel.send(createLastEtatMessage(undefined, actualUser, actuelPlayer, actualUserPoints, opponentUser, opponentPlayer, opponentUserPoints));
    } else {
        lastEtatFight = await lastEtatFight.edit(createLastEtatMessage(lastEtatFight, actualUser, actuelPlayer, actualUserPoints, opponentUser, opponentPlayer, opponentUserPoints));
    }

    if (logMessage === undefined) {
        logMessage = await message.channel.send(createLogMessage("Le combat vient de commencer !"));
    }

    if (lastMessageFromBot != undefined) {
        lastMessageFromBot.edit(await createFightMenu(actualUser));
    } else {
        lastMessageFromBot = await message.channel.send(await createFightMenu(actualUser));
        await addReactionsToFightMenu(lastMessageFromBot);
    }
    ...
    collector.on('collect', async (reaction) => {
        if (playerHasResponded) {
            if (reaction.users.last() === actualUser) { //On check que c'est le bon joueur qui réagis
                ...
                switch (reaction.emoji.name) {
                    case "🗡": //attaque rapide
                        ...
                        ({ defenderPower, attackerPower } = quickAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, logMessage));
                        break;
                    case "⚔": //attaque simple
                        // 100% des points d'attaque sont utilisés
                        // Taux de réussite de 60% qui monte à 70% sur un adversaire plus lent
                        ({ defenderPower, attackerPower } = simpleAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, logMessage));
                        break;
                    case "💣": //attaque ultime
                        ...
                        ({ defenderPower, attackerPower } = powerfullAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, logMessage));
                        break;
                    case "🛡": //defendre
                        // augmente la défense
                        ({ attackerDefenseAdd, defenderDefenseAdd } = ImproveDefense(actualUser, attacker, actuelPlayer, attackerDefenseAdd, logMessage, reaction, defenderDefenseAdd));
                        break;
                    default: // esquive
                        // augmente la vitesse de 30 points
                        ({ attackerSpeedAdd, defenderSpeedAdd } = ImproveSpeed(actualUser, attacker, actuelPlayer, attackerSpeedAdd, logMessage, reaction, defenderSpeedAdd));
                        break;
                }
            ...
            let messageReaction = await lastMessageFromBot.reactions.find(react=> react.emoji.name === reaction.emoji.name);
            messageReaction.remove(reaction.users.last())
        }
    });
    //end of the time the users have to answer to the demand
    collector.on('end', () => {
...
/**
 * Add fight choices reaction to a message
 * @param {*} message - The message where reactions will be added
 */
async function addReactionsToFightMenu(message) {
    await message.react("⚔");
    await message.react("🗡");
    await message.react("💣");
    await message.react("🛡");
...
function ImproveDefense(actualUser, attacker, actuelPlayer, attackerDefenseAdd, message, reaction, defenderDefenseAdd) {
    ...
    if (actualUser == attacker) {
        ...
        msg = reaction.emoji.name + Text.commands.fight.endIntroStart + actualUser.username + Text.commands.fight.defenseAdd + attackerDefenseAdd + Text.commands.fight.degatsOutro;
        ...
        msg = reaction.emoji.name + Text.commands.fight.endIntroStart + actualUser.username + Text.commands.fight.defenseAdd + defenderDefenseAdd + Text.commands.fight.degatsOutro;
        defenderDefenseAdd = Math.floor(defenderDefenseAdd * 0.5);
    }
    editLogMessage(msg, message);
...
function ImproveSpeed(actualUser, attacker, actuelPlayer, attackerSpeedAdd, message, reaction, defenderSpeedAdd) {
    ...
    if (actualUser == attacker) {
        ...
        msg = reaction.emoji.name + Text.commands.fight.endIntroStart + actualUser.username + Text.commands.fight.speedAdd + attackerSpeedAdd + Text.commands.fight.degatsOutro;
        ...
        msg = reaction.emoji.name + Text.commands.fight.endIntroStart + actualUser.username + Text.commands.fight.speedAdd + defenderSpeedAdd + Text.commands.fight.degatsOutro;
...
function powerfullAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, message) {
    ...
    editLogMessage(messageAttaqueUltime, message);
...
function simpleAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, message) {
    ...
    editLogMessage(messageAttaqueSimple, message);
    return { defenderPower, attackerPower };
}
...
function quickAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, message) {
    ...
    editLogMessage(messageAttaqueRapide, message);
    return { defenderPower, attackerPower };
}
 ...
 * @param {*} actualUser - The user that is currently playing
 */
async function createFightMenu(actualUser) {
    return new Discord.RichEmbed().setDescription(
        Text.commands.fight.menuStart + actualUser.username + Text.commands.fight.menuMiddle + Text.commands.fight.menuEnd);
}
