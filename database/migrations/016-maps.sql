...
CREATE TEMPORARY TABLE players_backup (id INTEGER PRIMARY KEY, score INTEGER NOT NULL, weeklyScore INTEGER NOT NULL, level INTEGER NOT NULL, experience INTEGER NOT NULL, money INTEGER NOT NULL, badges TEXT, entity_id INTEGER NOT NULL, guild_id INTEGER, updatedAt DATETIME, createdAt DATETIME, topggVoteAt DATETIME, nextEvent INTEGER, class INTEGER, pet_id INTEGER, last_pet_free DATETIME, effect_start_date DATETIME, effect_end_date DATETIME, previous_map_id INTEGER, map_id INTEGER, start_travel_date DATETIME, effect TEXT);
INSERT INTO players_backup SELECT id, score, weeklyScore, level, experience, money, badges, entity_id, guild_id, updatedAt, createdAt, topggVoteAt, nextEvent, class, pet_id, last_pet_free, DATETIME(0,'unixepoch'), -1, -1, DATETIME(0,'unixepoch'), DATETIME(0,'unixepoch'), ':smiley:' FROM players;
DROP TABLE players;
