...
export class MapLocationDataController extends DataController<number, MapLocation> {
    ...
    public getRandomGotoableMap(): MapLocation {
        if (this.missionsMapsCache === null) {
            this.missionsMapsCache = this.getValuesArray().filter((map) => map.canBeGoToPlaceMissionDestination);
        }
    ...
    public getMapsConnectedIds(mapId: number, blacklistId: number): number[] {
        return this.getValuesArray().filter(
    ...
    public getWithAttributes(attributes: string[]): MapLocation[] {
        return this.getValuesArray().filter((map) => attributes.includes(map.attribute));
    }
     ...
     * @param playerId
     */
    public async getPlayersOnMap(mapId: number, previousMapId: number, playerId: number): Promise<{ id: string }[]> {
