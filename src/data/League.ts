...
export class League extends Data<number> {
     ...
     */
    public generateRewardItem(): Promise<GenericItem> {
...
export class LeagueDataController extends DataController<number, League> {
    ...
    public getByEmoji(emoji: string): League {
        return this.getValuesArray().find((league) => league.emoji === emoji);
    }
    ...
    public getByGlory(gloryPoints: number): League {
        return this.getValuesArray().find((league) => league.minGloryPoints <= gloryPoints
