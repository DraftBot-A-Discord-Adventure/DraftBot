...
const fightCommand = async function(message, args, client, talkedRecently) {
  ...
  let spamchecker = 0;
  const attackerDefenseAdd = 20;
  const defenderDefenseAdd = 20;
  const attackerSpeedAdd = 30;
  const defenderSpeedAdd = 30;
  const nbTours = 0;
  // loading of the current player
  const player = await playerManager.getCurrentPlayer(message);
  ...
  if (player.level < DefaultValues.fight.minimalLevel) {
    ...
    if (playerManager.checkState(player, message, ':smiley:', language)) { // check if the player is not dead or sick or something else
      ...
      const messageIntro = await displayIntroMessage(message, attacker, chosenOpponent);
        ...
        if (fightIsOpen) {
          ...
          if (fightHasToBeCanceled(reaction)) {
            ...
            if (defender.id === message.author.id) { // le defenseur et l'attaquant sont la même personne
              ...
              const defenderPlayer = await playerManager.getPlayerById(defender.id, message);
              if (defenderPlayer.level < DefaultValues.fight.minimalLevel) {
                ...
                if (playerManager.checkState(defenderPlayer, message, ':smiley:', language, defender.username)) { // check if the player is not dead or sick or something else
                  ...
                  const actualUser = attacker;
                  const actuelPlayer = player;
                  const opponentUser = defender;
                  const opponentPlayer = defenderPlayer;
                  const lastMessageFromBot = undefined;
                  const lastEtatFight = undefined;
                  const attackerPower = player.getFightPower();
                  const defenderPower = defenderPlayer.getFightPower();
...
async function generateResetTopWeekEmbed(message) {
  ...
  const Text = await Tools.chargeText(message);
...
function getMinutesBeforeReset() {
  ...
  const dateOfReset = new Date(); // The next Sunday
  ...
  const nowMoment = new moment(now);
  const momentOfReset = new moment(dateOfReset);
...
async function displayIntroMessage(message, attacker, chosenOpponent) {
  const messageIntro = await generateIntroMessage(message, attacker, chosenOpponent);
...
function powerfullAttack(attackPower, player, opponentPlayer, actuelPlayer, defenderPower, attackerPower, attacker, actualUser, reaction, message) {
  ...
  const succes = Tools.generateRandomNumber(1, 100);
  let powerchanger = 0;
  ...
  const defensePower = opponentPlayer.defense;
  let degats = Math.round(attackPower - Math.round(defensePower));
  const random = Tools.generateRandomNumber(1, 8);
  if (degats > 0) {
    if (powerchanger == 2) {
...
async function displayFightMenu(message, actualUser) {
  const lastMessageFromBot = await message.channel.send(Text.commands.fight.menuStart + actualUser + Text.commands.fight.menuEnd);
...
* @return {Boolean} - true is the reaction is correct
*/
