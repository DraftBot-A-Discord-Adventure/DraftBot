...
const TopWeekCommand = async function(message, args, client) {
  ...
  const pageMax = Math.ceil(totalJoueur / DefaultValues.TopWeek.playersByPage);
  const page = getRequiredPageNumber(args);
  const erreur = testAbsurdsPages(message, page, pageMax);
  if (erreur == 0) {
    const bornesup = page * DefaultValues.TopWeek.playersByPage;
    const borneinf = bornesup - (DefaultValues.TopWeek.playersByPage - 1);
    const data = await playerManager.getTopWeekData(borneinf, bornesup);
...
const generateTopMessage = function(message, borneinf, bornesup, pageMax, page, actualPlayer, totalJoueur, data, client) {
  const messageTop = Text.commands.TopWeek.introDebut + borneinf + Text.commands.TopWeek.pageNumberSeparator + bornesup + Text.commands.TopWeek.introFin;
  const classementJoueur = actualPlayer.weeklyRank;
...
function getEndSentence(classementJoueur, actualPlayer, message, totalJoueur, page, pageMax) {
  ...
  if (classementJoueur != 1) {
    ...
    if (actualPlayer.weeklyScore > 0) {
      ...
      const pajejoueur = Math.ceil(classementJoueur / DefaultValues.TopWeek.playersByPage);
      if (page != pajejoueur) {
 ...
 * @return {String} - The time formatted in a string.
 */
function getResetDate() {
  ...
  const parsedTime = ' ' + diffDays + Text.commands.TopWeek.days + ' ' +
