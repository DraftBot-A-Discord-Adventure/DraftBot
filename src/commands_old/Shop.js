...
const PotionManager = require('../core/PotionManager');
...
const ShopCommand = async function(message, args, client, talkedRecently) {
  ...
  if (talkedRecently.has(message.author.id)) {
    ...
    if (playerManager.checkState(player, message, ':dizzy_face::zany_face::sick::sleeping::head_bandage::cold_face::confounded::clock2::smiley:', language)) {
      if (player.money < 0) {
        const ShopMessage = Text.commands.shop.errorEmoji + message.author.username + Text.commands.shop.noMoney;
        return message.channel.send(ShopMessage);
      }
      talkedRecently.add(message.author.id);
      const potionManager = new PotionManager();
      const dailyPotion = generateDailyPotion();
      const ShopMessage = generateShopMessage(dailyPotion, potionManager, language);
      message.channel.send(ShopMessage).then(async (msg) => {
        ...
        collector.on('collect', async (reaction) => {
          ...
          const potionPrice = dailyPotion.getValue() + parseInt(DefaultValues.shop.addedValue);
          ({choice, messageChoice} = saveChoice(reaction, choice, messageChoice, potionManager, dailyPotion, potionPrice));
          const messageconfirm = await displayConfirmMessage(message, messageChoice);
              ...
              switch (choice) {
                ...
                case 'guildXp':
                  if (player.money >= DefaultValues.shop.priceGuildXp) {
                    const guild = await guildManager.getCurrentGuild(message);
                    if (guild === null) {
                      message.channel.send(generateNotInAGuildException(message.author));
                      return;
                    }
                    const experience = Tools.generateRandomNumber(50, 450);
...
/**
 * @return {String} - A RichEmbed message wich display the NoUserException
...
async function sellDailyPotion(player, dailyPotion, potionPrice, playerManager) {
  ...
  const inventory = await inventoryManager.getInventoryById(player.id);
