...
const GuildCreateCommand = async (language, message, args) => {
  ...
  if (
    !(
      regexAllowed.test(askedName) &&
      !regexSpecialCases.test(askedName) &&
      askedName.length >= GUILD.MIN_GUILDNAME_SIZE &&
      askedName.length <= GUILD.MAX_GUILDNAME_SIZE
    )
  ) {
    return sendErrorMessage(
      message.author,
      ...
      if (reaction.first().emoji.name == MENU_REACTION.ACCEPT) {
        ...
        if (guild !== null) {
          ...
          return sendErrorMessage(
            ...
            message.channel,
            language,
            JsonReader.commands.guildCreate.getTranslation(language)
              .nameAlreadyUsed
          );
        }
        if (
