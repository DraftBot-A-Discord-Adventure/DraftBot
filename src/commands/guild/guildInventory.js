...
const GuildInventoryCommand = async (language, message, args) => {
  ...
  inventoryEmbed.setTitle(
    format(
      JsonReader.commands.guildInventory.getTranslation(language).embedTitle,
      {
        guild: guild.name,
      }
    )
  ...
  inventoryEmbed.addField(
    format(
      JsonReader.commands.guildInventory.getTranslation(language).foodTitle,
      {
        foodType:
          language === "fr"
    ...
    format(
      JsonReader.commands.guildInventory.getTranslation(language).foodField,
      {
  ...
  inventoryEmbed.addField(
    ...
    format(
      JsonReader.commands.guildInventory.getTranslation(language).growFoodField,
      {
        guildFood: guild.growFood,
        maxFood: GUILD.MAX_GROW_PETFOOD,
        emote: FOOD.GROW_FOOD.EMOTE,
        addGrowPoints: FOOD.GROW_FOOD.EFFECT,
      }
    ),
    true
  );
