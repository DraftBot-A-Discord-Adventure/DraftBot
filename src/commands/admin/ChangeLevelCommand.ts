...
function giveLevelsTo(entityToEdit: Entity, amount: number, interaction: CommandInteraction, language: string): void {
	...
	entityToEdit.Player.addExperience(Math.floor(entityToEdit.Player.getExperienceNeededToLevelUp() * ratioExpCurrentLevel) - entityToEdit.Player.experience, entityToEdit, interaction.channel, language, NumberChangeReason.ADMIN).then();
}
