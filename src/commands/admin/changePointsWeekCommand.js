...
const changePointsWeekCommand = async function (language, message, args) {
    ...
    .setAuthor(format(JsonReader.commands.pointsWeek.getTranslation(language).title, { pseudo: message.author.username }), message.author.displayAvatarURL())
    .setDescription(format(JsonReader.commands.pointsWeek.getTranslation(language).desc, { player: args[0] , points: args[1] }));
