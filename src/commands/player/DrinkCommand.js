...
const DrinkCommand = async (message, language, args) => {
	...
	if (potion.nature === NATURE.NONE && potion.id === JsonReader.models.inventories.potionId) {
		...
		removeBlockedPlayer(entity.discordUserId);
			if (args[0] === "force" || args[0] === "f" || validateMessage.isValidated()) {
				if (potion.nature === NATURE.NONE) {
					...
					return sendErrorMessage(message.author, message.channel, language, JsonReader.commands.drink.getTranslation(language).objectDoNothingError);
				}
