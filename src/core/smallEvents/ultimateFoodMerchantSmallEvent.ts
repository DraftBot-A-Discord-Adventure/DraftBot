...
export const smallEvent: SmallEvent = {
	...
	async executeSmallEvent(interaction: CommandInteraction, language: string, entity: any, seEmbed: MessageEmbed) {
		...
		async function giveReward(reward: any) {
			switch (reward.type) {
			case "ultimateFood":
				await giveFood(interaction, language, entity, interaction.user, Constants.PET_FOOD.ULTIMATE_FOOD, reward.option);
			...
			case "item":
				...
				break;
			case "commonFood":
				await giveFood(interaction, language, entity, interaction.user, Constants.PET_FOOD.COMMON_FOOD, reward.option);
