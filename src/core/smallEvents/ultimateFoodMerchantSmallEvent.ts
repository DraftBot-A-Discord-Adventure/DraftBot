...
export const smallEvent: SmallEvent = {
	...
	async executeSmallEvent(message: Message, language: string, entity: any, seEmbed: MessageEmbed) {
				...
				if (entity.Player.level >= 30) {
					if (RandomUtils.draftbotRandom.bool()) {
						if (guild.ultimateFood < foodData.getNumber("max.ultimateFood")) {
							console.log(foodData.getNumber("max.ultimateFood") - guild.ultimateFood);
							return {
								type: "ultimateFood",
		...
		async function giveReward(reward: any) {
			switch (reward.type) {
			case "ultimateFood":
				await giveFood(message, language, entity, message.author, JsonReader.food.ultimateFood, reward.option);
			...
			case "item":
				...
				break;
			case "commonFood":
				await giveFood(message, language, entity, message.author, JsonReader.food.commonFood, reward.option);
