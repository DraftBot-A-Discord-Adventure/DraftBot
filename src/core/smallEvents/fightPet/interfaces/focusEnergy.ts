...
/**
 * Focus energy if the player has a lot of energy, this has more chances to succeed
 */
export default class FocusEnergy extends FightPetAction {
	...
	public async applyOutcome(player: Player, feralPet: FeralPet): Promise<boolean> {
		...
		return RandomUtils.draftbotRandom.realZeroToOneInclusive() <=
			await player.getCumulativeFightPoint() / await player.getMaxCumulativeFightPoint() - feralPet.originalPet.rarity * SmallEventConstants.FIGHT_PET.ENERGY_BASED_ACTIONS_RARITY_MULTIPLIER;
	}
}