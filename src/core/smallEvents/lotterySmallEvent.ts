...
export const smallEvent: SmallEvent = {
	...
	async executeSmallEvent(interaction: CommandInteraction, language: string, player: Player, seEmbed: DraftBotEmbed): Promise<void> {
		...
		const collectorLottery = lotteryIntro.createReactionCollector({
			...
			filter: (reaction, user) => (emojiLottery.includes(reaction.emoji.name) || reaction.emoji.name === Constants.REACTIONS.NOT_REPLIED_REACTION) && user.id === interaction.user.id
		});
		...
		collectorLottery.on("end", async (collected) => {
			...
			if (!collected.first() || collected.first().emoji.name === Constants.REACTIONS.NOT_REPLIED_REACTION) {
			...
			if (RandomUtils.draftbotRandom.bool(dataLottery.getNumber(`successRate.${collected.first().emoji.name}`)) && (guild || reward !== SmallEventConstants.LOTTERY.REWARD_TYPES.GUILD_XP)) {
				...
				sentenceReward = format(translationLottery.getFromArray(collected.first().emoji.name, 0), {
					...
					xpWon: SmallEventConstants.LOTTERY.REWARDS.EXPERIENCE * coeff,
