...
declare function addBlockedPlayer(discordUserId: number, arg1: string, collector: any): void
declare function removeBlockedPlayer(discordUserId: number): void;
declare function minutesToString(minutes: number): string;
	...
	async executeSmallEvent(message: Message, language: string, entity: any, seEmbed: MessageEmbed) {
		...
		async function applyMalus(malus: any): Promise<void> {
			...
			case "time":
				...
				malus.option = minutesToString(malus.option);
				log(entity.discordUserId + "got a bad level small event and lost" + malus.option);
				break;
			case "nothing":
		...
		await builtEmbed.send(message.channel, (collector) => addBlockedPlayer(entity.discordUserId, "levelBad", collector));
	}
};